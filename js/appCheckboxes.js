var tweets = [],
	...
	refreshIntervalID = -1,
	maxTweetsShown = 100,
	tweetFetchURL = 'tweets/getTweets.php',
	currentURL = 'tweets/getTweets.php',
	approvedURL = 'tweets/approved.json',
		...
		<div class="content">' +
			'<div class="stream-item-header">\
				<div class="avatarAndCheckbox">' +
					checkboxString +
					'<a class="account-group" href="http://twitter.com/' + tweet.username + '">\
					<img src="' + biggerImage + '">\
				</div>\
...
function restoreChecked(){
	$.each($("input[type=checkbox]"), function(index, tweet){
		...
		if (checkedTweetIDs.indexOf(tweetID) != -1)
		{
			$(tweet).altCheckbox("clear");
			$(tweet).prop('checked', true);
			$(tweet).altCheckbox();
			//tweet.checked = true;
		}
		else
...
function saveClicked(){
    	...
    	if (checkboxes[i].checked){
    		if (tweetFetchURL == approvedURL)
    		{
    			...
    			if (approvedTweets[i] != null && approvedTweets[i] != ""){
    				//Ugh, looks like I need to escape characters myself
    				var aToPush = approvedTweets[i];
    				tweetsToSave.push(aToPush);
    			}
    		}
    		else if (fullTweetList[i] != null && fullTweetList[i] != ""){
    			//Ugh, looks like I need to escape characters myself
				var fToPush = fullTweetList[i];
    			tweetsToSave.push(fToPush);
    		}
    	}
	...
	$.post('./tweets/saveJSON.php', {json: encoded}, function(){
		location.reload();
	});
}
...
function checkAll(){
	//console.log($(".tweetCB").first());
	//console.log($(".tweetCB").first().prop('checked'));
	$(".tweetCB").altCheckbox("clear");
